pub mod lifo {
    //! This module implements a LIFO queue with enqueue and dequeue functionality.
    ...
    ///The LIFO queue with a pointer to its head and tail
    pub struct Lifo<T> {
        head: Link<T>,
        tail: Link<T>,
        length: i32,
...
mod tests {
    ...
    fn test_multi_item_lifo() {
        ...
        assert_eq!(5, l.dequeue().unwrap());
        assert_eq!(l.get_length(), i - 1);
        assert_eq!(4, l.dequeue().unwrap());
